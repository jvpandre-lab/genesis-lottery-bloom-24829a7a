...
function loadState(): ArbiterMemoryState {
  ...
  try {
    ...
    return {
      ...
      ...parsed,
      stats: {
        conservative: { ...DEFAULT_STATE.stats.conservative, ...parsed.stats?.conservative },
        hybrid: { ...DEFAULT_STATE.stats.hybrid, ...parsed.stats?.hybrid },
        aggressive: { ...DEFAULT_STATE.stats.aggressive, ...parsed.stats?.aggressive },
        exploratory: { ...DEFAULT_STATE.stats.exploratory, ...parsed.stats?.exploratory },
      },
...
function buildErrorKey(scenario: Scenario, chosen: string, rejected: string): string {
...
export const arbiterMemory = {
  ...
  getSummary(): {
    ...
    return {
      ...
      successRates: {
        conservative: { A: scoreRate(state.stats.conservative.A), B: scoreRate(state.stats.conservative.B) },
        hybrid: { A: scoreRate(state.stats.hybrid.A), B: scoreRate(state.stats.hybrid.B) },
        aggressive: { A: scoreRate(state.stats.aggressive.A), B: scoreRate(state.stats.aggressive.B) },
        exploratory: { A: scoreRate(state.stats.exploratory.A), B: scoreRate(state.stats.exploratory.B) },
      },
    };
  },

  registerDecision(record: Omit<ArbiterDecisionRecord, "id" | "createdAt">): string {
    ...
    if (!good) {
      const errorKey = buildErrorKey(record.context.scenario, record.chosen.brain, record.rejected.brain);
    ...
    if (state.decisions.length > MAX_DECISIONS) {
      state.decisions = state.decisions.slice(state.decisions.length - MAX_DECISIONS);
    }
    saveState(state);
    return id;
  },
  ...
  adjustBalanceA(balanceA: number, scenario: Scenario, picksA: number, picksB: number): number {
    ...
    return clamp(balanceA + adjustment, 0.10, 0.92);
  },
};
