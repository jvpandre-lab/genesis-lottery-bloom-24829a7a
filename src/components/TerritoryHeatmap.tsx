...
export function TerritoryHeatmap({ result }: { result: GenerationResult }) {
  const usage = useMemo(() => {
    ...
    for (const b of result.batches)
      for (const g of b.games) for (const n of g.numbers) u[n]++;
    return u;
  }, [result]);
    ...
    <div className="glass rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Ocupação do espaço 00–99 nesta geração.
          </p>
        </div>
        <div className="text-right">
          ...
          <div className="font-mono num-mono text-gradient text-lg">
            {entropyLabel}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mt-3 text-[10px] text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded ring-1 ring-destructive/60 inline-block" />{" "}
          Saturado
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded ring-1 ring-accent/60 inline-block" />{" "}
          Sub-explorado
        </span>
      </div>
    </div>
