...
interface StoredGen {
  ...
  batches: {
    name: BatchName;
    games: { numbers: Dezena[]; lineage: LineageId }[];
  }[];
}
      ...
      for (const g of gensRows ?? []) {
        ...
        for (const b of bs ?? []) {
          const { data: games } = await supabase
            ...
            .select("numbers, lineage")
            .eq("batch_id", b.id);
          batchesOut.push({
            name: b.name as BatchName,
            games: (games ?? []).map((x) => ({
              numbers: x.numbers as Dezena[],
              lineage: x.lineage as LineageId,
            })),
          });
        }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-semibold tracking-tight flex items-center gap-2">
            <History className="h-4 w-4 text-accent" /> Backtest histórico
          </h4>
            ...
            {report.windows.map((w) => (
              <div key={w.windowSize}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-base font-mono num-mono">
                      {w.avgHits.toFixed(2)}
                    </div>
            ...
            <Section title="Por lote">
              {report.perBatch.map((b) => (
                <RowItem
                  key={b.batch}
                  label={b.batch}
                  value={`${b.avgHits.toFixed(2)} (${(b.freq15plus * 100).toFixed(2)}% 15+)`}
                />
              ))}
            </Section>
          </div>
...
function Row({
  ...
  return (
    <div
      className={cn(
        "flex justify-between font-mono num-mono",
        highlight && "text-accent",
      )}
    >
      <span className="text-muted-foreground">{label}</span>
      <span>{(v * 100).toFixed(3)}%</span>
    </div>
  );
}
function Section({
