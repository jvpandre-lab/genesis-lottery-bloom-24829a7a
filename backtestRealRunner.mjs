import fs from 'fs';
import { backtestEvolutionaryRetrospective } from './src/engine/backtestEngine.js';
...
async function runBacktestValidation() {
  ...
  try {
    ...
    const errorLog = [`Erro fatal na validação: ${error.message}`, error.stack].join('\n');
    fs.writeFileSync('backtestRealValidation_error.txt', errorLog);
